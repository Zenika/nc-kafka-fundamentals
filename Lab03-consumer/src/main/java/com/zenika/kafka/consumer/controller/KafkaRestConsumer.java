...
public class KafkaRestConsumer {
    ...
    public SseEmitter consume() {
        ...
        sseExecutorService.execute(() -> {
                    // TODO Write code which subscribe on topic, consume record, send this one  over SSE and finally close consumer
                    boolean exitLoop = false;
                    while (!exitLoop) {
                        try {
                            for (ConsumerRecord<String, String> record : Collections.<ConsumerRecord<String, String>>emptyList()) {
